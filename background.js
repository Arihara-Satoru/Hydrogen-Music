...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    // 打印数据目录路径，方便用户排查问题
    console.log("[app] 应用名称:", app.getName());
    console.log("[app] 数据目录:", app.getPath("userData"));
    console.log("[app] 版本:", app.getVersion());
    ...
    try {
      ...
      const needsMigration =
        ...
        (existingSettings &&
          ...
          !existingSettings.music.level);

      if (needsMigration) {
        console.log(
          `[migration] 检测到版本变更或旧数据: 存储版本=${storedVersion || "无"}, 当前版本=${currentVersion}`,
        );

        if (existingSettings && typeof existingSettings === "object") {
          // 旧版 Netease 遗留数据没有 KuGou 特有的 music.level 字段，触发完全重置
          if (existingSettings.music && !existingSettings.music.level) {
            console.log(
              "[migration] 检测到旧版 Netease 数据格式，完全重置持久化数据",
            );
            ...
            // 同源版本升级：保留有效设置，仅更新版本标记并清理迁移字段
            existingSettings._appVersion = currentVersion;
            // 清理历史迁移标记字段
            if (existingSettings.music) {
              delete existingSettings.music.levelMigratedToLosslessV1;
            }
            await settingsStore.set("settings", existingSettings);
            console.log("[migration] 版本升级完成，已保留现有设置");
          }
        }

        // 清理可能不兼容的 localStorage 数据（通过渲染进程执行）
        // 注意：这里无法直接清理 localStorage，需要在渲染进程加载后处理
        // 但可以清理 electron-store 中已知的问题数据
        console.log("[migration] 数据迁移检查完成");
      }
    } catch (_) {}
...
const createWindow = (winstate = createMainWindowState()) => {
  ...
  const win = new BrowserWindow({
    ...
    titleBarOverlay: isMac
      ? { color: "#00000000", symbolColor: "#000000", height: 35 }
      : undefined,
  ...
  win.on("update-dock-menu", async (songInfo) => {
    ...
    if (songInfo && typeof songInfo === "object" && songInfo.name) {
      const title = songInfo.artist
        ? `${songInfo.name} - ${songInfo.artist} - Hydrogen Music`
        : `${songInfo.name} - Hydrogen Music`;
      win.setTitle(title);
    }
  });
    ...
    if (hasDevServer) {
      ...
      win.webContents.on(
        ...
        (_event, errorCode, errorDescription) => {
          console.warn(
            "Dev server not available, fallback to dist:",
            errorCode,
            errorDescription,
          );
          try {
...
const createLyricWindow = () => {
  ...
  if (hasDevServer) {
    ...
    lyricWin.webContents.on(
      ...
      (_event, errorCode, errorDescription) => {
        console.warn(
          "Dev server not available (desktop lyric), fallback to dist:",
          errorCode,
          errorDescription,
        );
        try {
          ...
          console.error("Fallback lyric loadFile failed:", e);
        }
      },
    );
  } else {
