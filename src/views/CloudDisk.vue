<script setup>
  ...
  const CLOUD_CATEGORY_MUSIC = 3
  const CLOUD_CATEGORY_VIDEO = 4
  const CLOUD_CATEGORY_ARCHIVE = 5
  const CLOUD_CATEGORY_DOCUMENT = 6

  // ponytail: KuGou's cloud API only exposes music; enable these tabs if the backend adds other file types.
  const supportsNonMusicFiles = false

  const CLOUD_CATEGORY_LABELS = {
    [CLOUD_CATEGORY_ALL]: '全部',
    [CLOUD_CATEGORY_IMAGE]: '图片',
    [CLOUD_CATEGORY_MUSIC]: '音乐',
    [CLOUD_CATEGORY_VIDEO]: '视频',
    [CLOUD_CATEGORY_ARCHIVE]: '压缩包',
    [CLOUD_CATEGORY_DOCUMENT]: '文档',
  }

  const imageExts = new Set(['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg', 'heic', 'heif', 'avif'])
  const musicExts = new Set(['mp3', 'aac', 'wma', 'wav', 'ogg', 'm4a', 'ape', 'flac', 'cue', 'aiff', 'aif', 'alac', 'dsf'])
  ...
  const supportedExt = musicExts
  const uploadAccept = [...supportedExt].map(ext => `.${ext}`).join(',')
  ...
  onMounted(async () => {
    changeHandler = async (e) => {
      ...
      const uploadedCount = await uploadFilesSequentially(files)
      // 清理 input 的值，释放文件引用并允许选择相同文件再次上传
      try { input.value = '' } catch (_) {}
      // 上传完成后刷新云盘信息
      if (uploadedCount > 0) await refreshCloudData()
    }
  ...
  function matchesCategory(item, category) {
    if (category == CLOUD_CATEGORY_ALL) return true

    const ext = extractCloudFileExt(item)

    if (category == CLOUD_CATEGORY_IMAGE) return imageExts.has(ext)
    if (category == CLOUD_CATEGORY_MUSIC) return musicExts.has(ext) || (!ext && hasSongId(item))
    if (category == CLOUD_CATEGORY_VIDEO) return videoExts.has(ext)
    if (category == CLOUD_CATEGORY_ARCHIVE) return archiveExts.has(ext)
    if (category == CLOUD_CATEGORY_DOCUMENT) return documentExts.has(ext)
  ...
  async function uploadSingle(file) {
    return uploadCloudSong(file)
  }

  async function uploadFilesSequentially(files) {
    const inputFiles = Array.from(files || [])
    const queue = inputFiles.filter(file => supportedExt.has(extractExt(file?.name)) && file.size > 0 && file.size <= maxUploadFileSize)
    if (!queue.length) {
      noticeOpen('请选择非空且不超过 100 MB 的支持音频文件', 2)
      return 0
    }
    if (queue.length !== inputFiles.length) noticeOpen('已跳过不支持、空白或超过 100 MB 的文件', 2)
    ...
    try {
      for (const file of queue) {
        try {
          ...
          console.error(`上传云盘歌曲失败: ${file.name}`, error)
          noticeOpen(`上传失败：${file.name}`, 3)
        }
      }
      if (queue.length > 1) noticeOpen(`上传完成：成功 ${uploadedCount} 个，失败 ${queue.length - uploadedCount} 个`, uploadedCount ? 2 : 3)
    } finally {
...
<template>
  <div class="cloud-disk" v-if=userStore.cloudDiskPage>
    <div class="disk-left">
      ...
      <div class="disk-tab">
        <div class="file-type">
          <div class="type" @click="changeCategory(1)">
            <div class="type-icon">
          ...
          <div v-if="supportsNonMusicFiles" class="type" @click="changeCategory(2)">
            <div class="type-icon">
          ...
          <div class="type" @click="changeCategory(3)">
            <div class="type-icon">
          ...
          <div v-if="supportsNonMusicFiles" class="type" @click="changeCategory(4)">
            <div class="type-icon">
          ...
          <div v-if="supportsNonMusicFiles" class="type" @click="changeCategory(5)">
            <div class="type-icon">
          ...
          <div v-if="supportsNonMusicFiles" class="type" @click="changeCategory(6)">
            <div class="type-icon">
        ...
        <div class="disk-info" v-if="cloudSongs">
          ...
          <div class="info-title">云盘信息</div>
            <div class="info-list">
              ...
              <div class="info-item" v-if="normalizeCapacityValue(maxSize) > 0">
              ...
              <div class="info-item">
                ...
                <div class="item-info" v-if="lastAddedTime">{{addTime(lastAddedTime)}}</div>
      ...
      >
        <Transition name="upload-fade">
          <div class="upload" @click.stop="uploadFile()" v-show="!isUploading">
            ...
            <input ref="uploadCloudDiskFile" style="display: none;" type="file" :accept="uploadAccept" multiple />
          </div>
        </Transition>
        <Transition name="upload-fade">
    ...
    <div class="disk-right">
      <CloudFileList
        :items="visibleCloudSongs"
        :category-key="typeSelect"
