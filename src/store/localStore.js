...
export const useLocalStore = defineStore("localStore", {
  ...
  actions: {
    ...
    updateDownloadList(list) {
      if (!this.downloadedFolderSettings) {
    ...
    getFolderSongs(arr, folderName) {
      arr.forEach((item) => {
        if (item.name == folderName) {
          ...
          this.getFolderSongs(item.children, folderName);
      });
    },
    ...
    updateLookupIndex(type, indexData = {}) {
      ...
      if (type == "downloaded") {
        ...
        nextLookupIndex.downloadedFoldersByPath = indexData.foldersByPath || {};
      }

      if (type == "local") {
        nextLookupIndex.localFoldersByName = indexData.foldersByName || {};
        nextLookupIndex.localFoldersByPath = indexData.foldersByPath || {};
    ...
    getSongSearchText(song, scope = null) {
      ...
      return (
        this.lookupIndex.songSearchByScope?.local?.[songId] ||
    ...
    updateLocalMusicDetail(type, query, id) {
      ...
      if (type == "localFiles") {
        const folderScope =
          query?.type == "downloaded" ? "downloaded" : "local";

        // 优先使用 dirPath 查找（唯一），其次按 name 查找
        let folderEntry = null;
        if (query?.dirPath) {
          folderEntry =
            folderScope == "downloaded"
              ? this.lookupIndex.downloadedFoldersByPath?.[query.dirPath]
              : this.lookupIndex.localFoldersByPath?.[query.dirPath];
        }
        if (!folderEntry && query?.name) {
          folderEntry =
            folderScope == "downloaded"
        ...
        this.currentSelectedInfo = {
          ...
          dirPath: folderEntry?.dirPath || query?.dirPath || null,
          scope: folderScope,
        };
        this.currentSelectedSongs = Array.isArray(folderEntry?.songs)
          ? folderEntry.songs
          : [];
      }
      if (type == "localAlbum") {
        ...
        this.currentSelectedSongs = Array.isArray(albumEntry?.songs)
          ? albumEntry.songs
          : [];
        if (
          Array.isArray(this.currentSelectedSongs) &&
          this.currentSelectedSongs.length > 0
        )
      ...
      if (type == "localArtist") {
        ...
        this.currentSelectedSongs = Array.isArray(artistEntry?.songs)
          ? artistEntry.songs
          : [];
        if (
          Array.isArray(this.currentSelectedSongs) &&
