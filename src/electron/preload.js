...
contextBridge.exposeInMainWorld("windowApi", {
  ...
  getLocalMusicLyric: (filePath, options) =>
    ipcRenderer.invoke("get-local-music-lyric", filePath, options),
  copyTxt,
  checkUpdate,
  ...
  onResetLocalStorage: (callback) => {
    ...
    ipcRenderer.on("reset-local-storage", listener);
    return () => ipcRenderer.removeListener("reset-local-storage", listener);
  },
});
...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  setLyricWindowAspectRatio: (aspectRatio) =>
    ipcRenderer.send("set-lyric-window-aspect-ratio", { aspectRatio }),
  getLyricWindowContentBounds: () =>
    ipcRenderer.invoke("get-lyric-window-content-bounds"),
  moveLyricWindowContentTo: (x, y, width, height) =>
    ipcRenderer.send("move-lyric-window-content-to", { x, y, width, height }),
});
...
contextBridge.exposeInMainWorld("playerApi", {
  ...
  onVolumeChanged: (callback) =>
    ipcRenderer.on("volume_changed", (_, volume) => {
      callback(volume);
    }),
});
