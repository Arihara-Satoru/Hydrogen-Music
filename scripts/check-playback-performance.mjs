...
const [widget, playerView, lyric, visualizer, desktopLyric, player] = await Promise.all([
  ...
  readSource("../src/components/DesktopLyric.vue"),
...
assert.doesNotMatch(visualizer, /levels\.value/);
assert.match(visualizer, /visualizerRef\.value\?\.children/);

assert.doesNotMatch(desktopLyric, /startScanAnimation|--scan-progress|--text-clip/);
assert.match(desktopLyric, /animation:\s*desktop-lyric-scan-bar 10s linear infinite/);
assert.match(desktopLyric, /animation:\s*desktop-lyric-scan-text 10s linear infinite/);
